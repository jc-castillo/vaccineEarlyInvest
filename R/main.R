...
#' Optimal portfolio with limited budget
#'
#' Computes optimal portfolios for a price-taking country when there is a limited budget at
#' different levels
#'
#' @param parameters `Parameters` object with model parameters
#' @param population Country population (in millions)
...
#' @param loss2yr Cumulative percent of GDP lost because of pandemic over two years
#' @param price Market price for manufacturing capacity ($ per course / year)
#' @param shadowCosts Vector with shadow cost of public funds
#' @param candidateFile File with candidate data
#' @param inisteps Step sizes to optimize over for the initial point
#' @param mainstep Step size for main optimization
#' @param candidateFile File with candidate data
...
#'                    demand <- demandPriceTaker(parameters=par)
#' }
limBudgetPriceTaker <- function(parameters=NULL, population, gdp_pc, frac_high_risk, loss2yr, price,
                             shadowCosts=seq(1,5,0.1), inisteps=c(10,1), mainstep=0.1, candidateFile=NULL, verbose=0) {
  ...
  # Creating objective function
  objectiveFun <- function(capacities, grid, price, par, lambda) {
  ...
  for (s in inisteps) {
    capacities <- optimizeGrid(capacities, objectiveFun, verbose=verbose, step=s, price=price, par=par, lambda=sc)
  }

  # Setting up objects to save infomation
  optimizations <- data.table(shadowCost=shadowCosts)
  optimizations[, ind := .I]
  setkey(optimizations, ind)
  allCapacities <- matrix(0, length(shadowCosts), length(capacities))

  # Loop over shadowCosts to get optimal portfolios
  for (sc in shadowCosts[1:length(shadowCosts)]) {
    capacities <- optimizeGrid(capacities, objectiveFun, verbose=verbose, step=mainstep, price=price,
                               par=par, lambda=sc)

    i <- optimizations[shadowCost==sc, ind]
    allCapacities[i, ] <- capacities
    optimizations[.(i), expBenefit :=
                    countryExpectedBenefits(capacities, dcandidate, targetPermutations, dplatforms, par, grid=mainstep)]
    optimizations[.(i), cost := priceTakerCost(capacities, price)]
