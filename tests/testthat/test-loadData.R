...
test_that('loadData report error if the includeVaccines contains vaccines that are not prescribed',{
  par = Parameters$new()
  expect_error(loadData(par = par, candidateFile = 'extdata/vaccinesSummary.csv',includeVaccines = 1))
  expect_error(loadData(par = par, candidateFile = 'extdata/vaccinesSummary.csv',includeVaccines = c('BCG',123)))
  expect_error(loadData(par = par, candidateFile = 'extdata/vaccinesSummary.csv',includeVaccines = 'asd'))
})


test_that('includeVaccines works',{
  par = Parameters$new()
  d0 = loadData(par = par, candidateFile = 'extdata/vaccinesSummary.csv')
  d1 = loadData(par = par, candidateFile = 'extdata/vaccinesSummary.csv',includeVaccines = c('BCG'))
